...
doc_type: reference
builds: [erp, mtk]
related: [forms, placeholders, landings, user-fields, push-notifications, glossary, mechanics-pwa, campaigns, custom-fields, server]
language: ru
updated: 2026-08-12
---
...
- *Форма не отправляется* — если форма не работает (часто корень в макросе)
...
- Отдельным схемам флоу нужно именно `false` — разбор в *уточните у поддержки*.
...
Если после отключения `fbPixel` события всё равно двоятся, источник дубля не в SDK: их шлёт либо сторонний пиксель на лэнде, либо сам рекламодатель. Диагностика — *FB / Meta — токен инвалидируется, атрибуция, косты*.
...
| `form-translations` | `translations: {...}` | Переопределить/дополнить переводы формы. |
...
Ядро обмена с агентом (`aio-exchange.js`):
...
- **Поля визита.** `trigger(key, value)` копит значения в буфер. Буфер сбрасывается на сервер:
  ...
  - Эндпоинт: `GET /api/v1/trigger/field/<visit_uuid>/?<key>=<value>&…`
- **Конверсии.** `conversion(typeUUID, query)` шлёт **сразу**:
  - Эндпоинт: `GET /api/v1/trigger/conversion/<visit_uuid>/<conversion_type_uuid>/?<query>`
  - `query` — произвольные параметры (например, сумма/валюта), уходят в query-строке.

> Практический вывод: то, что известно **сразу на загрузке** (`landed`, размеры окна, таймзона, `fingerprint`, стартовый нулевой скролл), уходит первым сбросом сразу после инициализации макросов — ждать 10 секунд не нужно. А вот значения, которые копятся **по ходу** (вехи времени на лэнде, рост скролла, поля формы по `blur`, куки `_fbp` / `_fbc` / `_ttp`), появляются с задержкой до 10 секунд или при уходе со страницы.
...
- Встроены переводы 30+ языков (ключи `FIRST_NAME`, `LAST_NAME`, `EMAIL`, `PHONE`, `SUBMIT`, `SUBMIT_LOADING`, `SUBMIT_SUCCESS`, `SUBMIT_STEP`, `HEADER`).
- Язык формы = `form.language` (дефолт — первые 2 буквы `navigator.language`). Фолбэк: нужный язык → `en` → сам ключ.
- Переопределить/добавить переводы — через `aioBus` тип `form-translations`.
...
PWA-макрос превращает лэнд в устанавливаемое приложение и подписывает на web-push. Работает по DOM-контейнерам (id настраиваются): кнопка install (`installButtonId`, дефолт `install`), контейнеры `pwa-install` / `pwa-preloader` / `pwa-inside`, `intent-modal`. Скриншоты и иконки install-UI берутся из **серверного manifest-JSON** (не из DOM-слайдера) и заранее префетчатся, чтобы rich-диалог установки Chrome собрался в окне user-activation (`sliderItemsQuerySelector` в текущем бандле не используется). Manifest не пишется в макросе — он собирается **на сервере per-visit** и подключается макросом как `<link rel=manifest href=/api/v1/pwa/manifest/<visit_uuid>.json>` (стабильный URL, не blob → install-identity персистит).

Три ветки поведения:
- **Доступно для установки** (Android + Chrome, не webview, не внутри PWA): показывает preloader → ловит `beforeinstallprompt` → активирует install-кнопку → по клику `prompt()` → анимация прогресса установки. На `appinstalled` шлёт конверсию `conversions.install`.
...
- **Внутри уже установленной PWA**: шлёт конверсии `conversions.open` **и** `conversions.install` (обе, на каждом открытии); регистрирует service worker `/aio-static/aio-sw.js`; запрашивает разрешение на уведомления; если визит ещё не подписан (`aio.visit.is_push_subscribed` ложно) — берёт VAPID-ключ `GET /api/v1/agent/pub-key-push-settings/<visit_uuid>`, делает `pushManager.subscribe`, шлёт подписку `POST /api/v1/agent/persist-push-settings/<visit_uuid>`, затем конверсию `conversions.subscribe`; в конце редиректит на «hess»-URL. - `conversions` — это UUID типов конверсий, которые шлются в моменты install / open / subscribe.

#### Почему install-запрос виден на каждом открытии PWA

При открытии уже установленной PWA макрос шлёт **обе** конверсии — `open` и `install`. Второй `Install` при этом не создаётся: тип `Install` — `Single Conversion` (одна на визит), повторный запрос отсекается на стороне AIO. То есть в сетевых логах лэнда install-запрос виден каждый раз, а в конверсиях визита остаётся один — это стоит держать в голове при разборе трафика.
...
- `isInsidePWA()`, `isAvailableForInstall()` (Android+Chrome), `isAvailableForIntent()` (Android webview).
...
|---|---|
| Записать поле визита | `GET /api/v1/trigger/field/<visit_uuid>/?<k>=<v>` |
| Отправить конверсию | `GET /api/v1/trigger/conversion/<visit_uuid>/<type_uuid>/?<query>` |
...
- **Версии бандла.** Лэнды могут грузить разные версии `main.js` (видно по `?v=` в `<script>`). При расследовании «почему опция не работает» — сверяй поведение с версией бандла на конкретном лэнде.
