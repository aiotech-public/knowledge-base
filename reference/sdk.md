...
doc_type: reference
builds: [erp, mtk]
related: [forms, placeholders, landings, user-fields, push-notifications, glossary, mechanics-pwa, campaigns, custom-fields, server, how-to-pwa, notifications-flow, conversion-model]
language: ru
updated: 2026-08-12
---
...
- **Поля визита.** `trigger(key, value)` копит значения в буфер. Буфер сбрасывается на сервер:
  ...
  - и дополнительно на событие `pagehide` (уход со страницы) — доставка этого последнего сброса не гарантирована (см. ниже).
...
Сброс на `pagehide` — **best-effort**: он уходит обычным запросом и только если предыдущий сброс уже завершился, поэтому последний буфер может и не доехать. Практический вывод для разбора: отсутствие «поздних» полей у визита, который быстро ушёл со страницы, — ожидаемая картина, а не поломка.
...
- **На iOS**: install-кнопка вместо системного prompt открывает модалку с инструкцией добавить страницу на домашний экран (разобрана ниже).
...
Отличие от полей визита: поля копятся в буфере, и их последний сброс на уходе со страницы не гарантирован (§II.4), а конверсия уходит сразу и переживает редирект.
