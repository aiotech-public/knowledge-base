...
description: Что такое визит в AIO как сущность — одна браузерная сессия в кампании (cuuid+suuid+cookie), живой инстанс, идущий по Flow и накапливающий поля; две ортогональные оси (Flow State vs Visit Status), связь с конверсией. Модель-хаб; процесс — visit-lifecycle.md, данные — visit-field.md.
doc_type: model
builds: [erp, mtk]
related: [visit-lifecycle, visit-field, conversion-model, server, domain-model, debug-with-logs, flow-model, landings, campaign, source]
language: ru
updated: 2026-08-11
---
...
Визит опознаётся **парой `cuuid`+`suuid`** (в какую кампанию и через какой source) плюс **cookie** в браузере посетителя. Новый визит — другой браузер / инкогнито / очистка кук. Окно уникальности (скользящие 30 дней, `Rolling Window 30d`) и полная механика идентификации, дубли (сессия vs fingerprint) — [mechanics/visit-lifecycle.md](../mechanics/visit-lifecycle.md). Нет `cuuid`/`suuid` → Direct Traffic (`Default Query` / `Direct Traffic Distribution`).

Как ещё называют: «уники», «уникальные визиты» — про уникальность визита (окно `Rolling Window 30d`).
...
Эти оси **не связаны жёстко**: визит может быть `Live`, стоя на Offer, или `Left`, бросив на Preland. Связь «шаги флоу ↔ текущий стейт визита» детально — [models/domain-model.md](domain-model.md) → «Как связаны Flow States и текущий стейт Visit»; статусы — *Visit Status = Wait — что это значит*.
...
На каждом шаге визит может «не дойти»: JS не догрузился, фильтр зарубил (`Rejected` → White), антифрод срезал перед пушем, дестинейшн отверг, правила Content не подошли (`No payload`). Сумма таких потерь — метрика `Visit Loss`. Поэтому дебаг визита = пройти его путь и найти точку обрыва (см. [mechanics/visit-lifecycle.md](../mechanics/visit-lifecycle.md) → потеря).
...
- **`Wait` ≠ «сломалось»** — часто визит просто ждёт первые ивенты от SDK (запись сессии не началась; см. *Visit Status = Wait — что это значит*).
