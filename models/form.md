...
**Layouts** отвечают исключительно за визуальное расположение. Layout = массив массивов: внутри одного массива поля идут side-by-side, между массивами — на новой строке. Ключевое следствие: если push успешен, но у получателя данные перепутаны (firstname вместо телефона) — виноваты `key` контролов, не Layouts; чинить в Controls. См. *Форма не отправляется*.

## Как работают Languages — мульти-язычные переводы формы

Один Base Language + N additional; форма авто-переключается по browser language визита. Встроены переводы 30+ языков (полный список ключей строк — [reference/sdk.md](../reference/sdk.md), раздел «i18n и переводы формы»). Фолбэк: нужный язык → `en` → сам ключ. Кастомные переводы **не** копируются из SDK defaults автоматически.
...
Успешный submit пушит лид в Destination (через `_aio_handle=form` + handle); `rejectedMessage` показывается при `success:false`; проверки перед пушем могут отрезать лид (*уточните у поддержки*). → [models/destination.md](destination.md), *Форма не отправляется*
...
На сабмите лид проходит встроенный `AIO Antifraud` — проверки перед пушем в Destination; их состав и настройка описаны в *уточните у поддержки*.
...
- **Версии бандла (`?v=`) могут различаться** — поведение опции сверять с версией SDK на конкретном лэнде.
...
- **Диагностика «форма не отправляется»** → *Форма не отправляется*.
